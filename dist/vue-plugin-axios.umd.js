(function (global, factory) {
	...
	(factory((global['vue-plugin-axios'] = {})));
	...
	function requestPromiseWrap(method, arg) {
	  return this[method].apply(this, _toConsumableArray(arg)).then(function (_ref) {
	    var data = _ref.data;
	    return data;
	  }); // .catch(e => {
	  //   // throw e
	...
	var plugin = {
	  install: function install(Vue) {
	    ...
	    var axios = options.axios,
	        ...
	        _options$nuxtInject = options.nuxtInject,
	        nuxtInject = _options$nuxtInject === void 0 ? null : _options$nuxtInject;
	...
	function Module(axiosPackage, config, other) {
	  ...
	  var requests = ['post', 'get', 'put', 'patch', 'delete', 'head', 'options'];
	  var requestsObj = {};
	    ...
	    Object.defineProperty(requestsObj, '$' + name, {
	      ...
	      get: function get() {
	        return function () {
	          ...
	          return requestPromiseWrap.apply(self.axiosInstance, [name, arg]);
	        };
	      }
	    });
	  });

	  var publicObj = _objectSpread({}, requestsObj, {
	    // добавляет в публичный объект методы для удобных запросов, с легким получением data
	    get $axios() {
